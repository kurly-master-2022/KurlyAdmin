...
class MetricWorkflowApi(
    ...
    fun putMetricRequest(metric: Metric): Boolean{
        ...
        return when(val responseCode = HttpStatus.valueOf(requestResult.statusCode())){
            HttpStatus.OK -> { this.logger.info("Metric $metric 의 워크플로 생성이 완료되었습니다!"); true }
            HttpStatus.BAD_REQUEST -> { this.logger.error("Metric $metric 의 값이 올바르지 않습니다."); false }
            HttpStatus.INTERNAL_SERVER_ERROR -> { this.logger.error("Metric $metric 생성 과정이 실패했습니다!"); false }
            else -> {
    ...
    fun deleteMetricWorkflow(metric: Metric): Boolean {
        ...
        return when(HttpStatus.valueOf(requestResult.statusCode())){
            HttpStatus.OK -> { this.logger.info("Metric $metric 의 워크플로 삭제가 완료되었습니다!"); true }
            HttpStatus.NOT_FOUND -> { this.logger.error("Metric $metric 이 워크플로우에 존재하지 않습니다!"); false }
            else -> { this.logger.error("Metric $metric 의 삭제 중 알 수 없는 오류가 발생했습니다!"); false }
        }
    }
}